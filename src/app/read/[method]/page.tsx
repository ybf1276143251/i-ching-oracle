  ...
  return (<div className="relative min-h-screen pt-28 pb-24 px-6"><PremiumModal open={showModal} onClose={()=>setShowModal(false)}/>
    <div className="max-w-2xl mx-auto relative z-10">
      ...
      <AnimatePresence>{result&&(<motion.div ref={ref} initial={{opacity:0,y:40}} animate={{opacity:1,y:0}} transition={{duration:0.7}}>
        ...
        {!fullText?(<div className="glass p-8 text-center"><p className="text-[var(--text-secondary)] mb-4">{isPremium?(lang==="en"?"Unlock full AI reading.":"解锁完整AI解读。"):(lang==="en"?"Upgrade to Pro for full reading.":"升级Pro获取完整解读。")}</p><div className="flex flex-col sm:flex-row gap-3 justify-center"><button onClick={unlock} disabled={interpreting} className="btn btn-primary">{interpreting?<>☯...</>:<>释 {t.unlockFull}</>}</button>{!isPremium&&<button onClick={()=>setShowModal(true)} className="btn btn-secondary">乾 {t.upgradePremium}</button>}</div></div>):(<motion.div initial={{opacity:0}} animate={{opacity:1}} className="glass p-8"><h3 className="text-lg font-semibold text-gradient mb-6">{t.fullInterpretation}</h3><div className="prose text-sm" dangerouslySetInnerHTML={{__html:fullText.replace(/\n/g,"<br/>").replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>")}}/></motion.div>)}
